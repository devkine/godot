...
bool atom_emission_enabled() {
	return bool(scene_data.atom_emission_flags & ATOM_EMISSION_FLAG_ENABLED);
}

bool atom_emission_texture_as_energy_filter() {
	return bool(scene_data.atom_emission_flags & ATOM_EMISSION_FLAG_TEXTURE_AS_ENERGY_FILTER);
}

float atom_emission_nits_to_energy(float p_emission_nits) {
	return clamp(p_emission_nits, 0.0, scene_data.atom_emission_max_nits);
}

float atom_resolve_emission_energy(float p_emission_energy) {
	if (bool(scene_data.atom_emission_flags & ATOM_EMISSION_FLAG_PHOTOMETRIC_LUMINANCE)) {
...
vec3 atom_cap_emission_preserve_hue(vec3 p_emission) {
	...
	if (peak <= scene_data.atom_emission_max_nits) {
		return p_emission;
	}

	return p_emission * (scene_data.atom_emission_max_nits / peak);
}
...
vec3 atom_apply_filmic_emission_bias(vec3 p_emission) {
	...
	float soft_knee = max(scene_data.atom_emission_default_nits * scene_data.atom_emission_filmic_exposure_multiplier, 0.001);
	if (peak <= soft_knee) {
		return p_emission;
	}

	float range = max(scene_data.atom_emission_max_nits - soft_knee, 0.001);
...
vec3 atom_apply_shared_emission_response(vec3 p_emission, float p_alpha) {
	...
	if (bool(scene_data.atom_emission_flags & ATOM_EMISSION_FLAG_FILMIC_RESPONSE)) {
...
// If alpha scissor is used, we must further the edge threshold, otherwise we won't get any edge feather
	...
	emission = hvec3(atom_apply_shared_emission_response(vec3(emission), float(alpha)));
#endif
